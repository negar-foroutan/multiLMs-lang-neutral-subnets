...
""" Lottery Ticket Hypothesis (LTH) for the XNLI task and for mT5 model """
...
from helpers.pruning_utils import rewind_mt5_model, pruning_mt5_model, see_mt5_weight_rate
...
class LTHTrainingArguments:
    ...
    max_source_length: int = field(
        default=128,
        metadata={"help": "The maximum total input sequence length after tokenization.Sequences longer than this will "
    ...
    model_language: str = field(
        default=None,
        metadata={"help": "Model language or type."}
    )
    pruning_type: str = field(
        default=None,
        metadata={"help": "Pruning type."}
    )
    mask_net_task: str = field(
        default="XNLI",
        metadata={"help": "Pruning type."}
    )
    ...
    evaluate_during_training: bool = field(
        default=True,
        metadata={"help": "Whether to evaluate during training or not."}
    )
    ...
    dataset_name: Optional[str] = field(
        default="xnli", metadata={"help": "The configuration name of the dataset to use (via the datasets library)."}
    )
...
def train(args, train_dataset, eval_dataset, model, data_collator, tokenizer, orig):
    """ Train the model """
    record_result = []
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    pruning_mt5_model(model, 1/(10-pruning_step))
                    rate_weight_equal_zero = see_mt5_weight_rate(model)
                    pruning_step += 1
...
def main():
    ...
    logger.info("********** XNLI LTH pruning for {} language and MT5 model. **********".format(args.data_language))
    ...
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Make sure only the first distributed training process will download model/vocab
    ...
    model = torch.nn.DataParallel(model)
    origin_model_dict = rewind_mt5_model(model.state_dict())

    if args.local_rank == 0:
        torch.distributed.barrier()  # Make sure only the first process in distributed training will download model/vocab

    # Change the input data format to the format of mT5
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        model_inputs = tokenizer(examples["input_seq"], max_length=args.max_source_length,
                                 padding=padding, truncation=True)
        with tokenizer.as_target_tokenizer():
            labels = tokenizer(examples["target_seq"], max_length=args.max_target_length,
                               padding=padding, truncation=True)
