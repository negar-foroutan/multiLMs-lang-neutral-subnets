...
""" Finetuning the library models for NER task """
...
from helpers.pruning_utils import pruning_model, see_weight_rate, pruning_model_custom, random_pruning_model
...
class LTHTrainingArguments:
    ...
    save_steps: int = field(
        default=1875, metadata={"help": "Save checkpoint every X updates steps."}
    )
    logging_steps: int = field(
        default=1875, metadata={"help": "Log every X updates steps."}
    )
    ...
    initial_weight: str = field(
        default="pre",
        metadata = {"help": "Whether start with pre-trained or random weights"},
    )
    ...
    dataset_name: Optional[str] = field(
        default="wikiann", metadata={"help": "The configuration name of the dataset to use (via the datasets library)."}
    )
...
def main():
    ...
    logger.info("***********************************************")
    logger.info(f"* NER fine-tuning for {args.data_language} data.")
    ...
    if args.do_train:
        ...
        print("Train datset size: {}".format(len(train_dataset)))
    ...
    if args.do_eval:
        ...
        print("Evaluation datset size: {}".format(len(eval_dataset)))
    ...
    # Training
    if args.do_train:
        global_step, tr_loss = train(args, train_dataset, eval_dataset, model, data_collator, compute_metrics, tokenizer)
